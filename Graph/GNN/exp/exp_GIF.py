...
from lib_gnn_model.node_classifier_hsic import HSICNodeClassifier

import pdb


class GraphExpGraphInfluenceFunction(Exp):
    def __init__(self, args):
        super(GraphExpGraphInfluenceFunction, self).__init__(args)
        ...
        print(run_f1_unlearning)
        self.logger.info(f"[unlearn] f1_score of {self.args['method']}: avg={f1_score_unlearning_avg}, std={f1_score_unlearning_std}")
        self.logger.info(f"[unlearn] {self.args['method']} unlearing time: avg={np.average(unlearning_time_avg)} seconds")
    ...
    def unlearning_request(self):
        ...
        self.data.y_unlearn = self.data.y.clone()
    ...
    def determine_target_model(self):
        ...
        if self.args["method"].lower() == "hsic":
            self.target_model = HSICNodeClassifier(self.num_feats, num_classes, self.args)
        elif self.args["method"].lower() in ["gif", "if"]:
    ...
    def gif_approxi(self, res_tuple):
        ...
        if self.args["method"] in ["GIF", "HSIC"]:
        ...
        for x in h_estimate:
            if torch.all(torch.isnan(x)) or torch.all(torch.isinf(x)):
                print("\033[31m [gif_approxi] h_estimate has NAN value \033[0m")
                pdb.set_trace()
        ...
        for i in range(iteration):
            ...
            for x in h_estimate:
                if torch.all(torch.isnan(x)) or torch.all(torch.isinf(x)):
                    print(f"\033[31m [gif_approxi] h_estimate_2 has NAN value, iteration {i} \033[0m")
                    pdb.set_trace()
                
        params_change = [h_est / scale for h_est in h_estimate]
        params_esti   = [p1 + p2 for p1, p2 in zip(params_change, model_params)]
        
        for x in params_change:
            if torch.all(torch.isnan(x)) or torch.all(torch.isinf(x)):
                print("\033[31m [gif_approxi] params_change has NAN value \033[0m")
                pdb.set_trace()
        
        for x in params_esti:
            if torch.all(torch.isnan(x)) or torch.all(torch.isinf(x)):
                print("\033[31m [gif_approxi] params_esti has NAN value \033[0m")
                pdb.set_trace()
