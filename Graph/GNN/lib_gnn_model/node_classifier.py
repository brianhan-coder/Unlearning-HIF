...
from lib_gnn_model.gin.gin_net_batch_no import GINNet
...
class NodeClassifier(GNNBase):
    ...
    def train_model(self, unlearn_info=None):
        ...
        for epoch in range(self.args['num_epochs']):
            ...
            #self.logger.info(f'Train: {train_acc:.8f}, Test: {test_acc:.8f}')
        ...
        if self.args["method"] in ["GIF", "IF"]:
            ...
            if torch.all(torch.isnan(out2)) or torch.all(torch.isinf(out2)):
                print("[train_model] FALSE, OUT is all NAN")
                pdb.set_trace()
    ...
    def evaluate_unlearn_F1(self, new_parameters):
        ...
        for p in self.model.parameters():
            if torch.all(torch.isnan(new_parameters[idx])) or torch.all(torch.isinf(new_parameters[idx])):
                print("\033[31m [evaluate_unlearn_F1] new_parameters has NAN value \033[0m")
                pdb.set_trace()
        ...
        if torch.all(torch.isnan(out)) or torch.all(torch.isinf(out)):
            print("\033[31m [evaluate_unlearn_F1] FALSE, OUT is all NAN \033[0m")
            pdb.set_trace()
...
if __name__ == '__main__':
    ...
    dataset_name = 'cora'
    dataset = Planetoid("temp_data/raw_data/", dataset_name, transform=T.NormalizeFeatures())
    data = dataset[0]
