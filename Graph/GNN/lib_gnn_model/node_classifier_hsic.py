...
class HSICNodeClassifier(GNNBase):
    def __init__(self, num_feats, num_classes, args, data=None):
        super(HSICNodeClassifier, self).__init__()
        ...
        self.model = self.determine_model().to(self.device)
        self.data = data
    ...
    def determine_model(self):
        ...
        if self.target_model == 'SAGE':
            self.lr, self.decay = 0.05, 0.001
            return SageNet(self.num_feats, 256, self.num_classes)
        elif self.target_model == 'GAT':
            self.lr, self.decay = 0.05, 0.001
            return GATNet(self.num_feats, self.num_classes)
        elif self.target_model == 'GCN':
            self.lr, self.decay = 0.05, 0.0001
            return GCNNet(self.num_feats, self.num_classes)
        elif self.target_model == 'GIN':
            self.lr, self.decay = 0.05, 0.0001
            return GINNet(self.num_feats, self.num_classes)
        elif self.target_model == 'SGC':
            self.lr, self.decay = 0.05, 0.0001
            return SGCNet(self.num_feats, self.num_classes)
        else:
            raise Exception('unsupported target model')
            
    def cal_hsic(self, x, y):
        kx = self.cal_k_matrix(x)
        ky = self.cal_k_matrix(y)
        n = kx.shape[0]
    ...
    def cal_k_matrix(self, x, sigma=1.0):
        """
    ...
    def train_model(self, unlearn_info=None):
        ...
        self.data.y_unlearn = self.data.y_unlearn.squeeze().to(self.device)
        ...
        for epoch in range(self.args['num_epochs']):
            ...
            for batch_size, n_id, adjs in self.train_loader:
                ...
                # loss1 = numda * (
                #     self.cal_hsic(self.data.x[n_id[:batch_size]], self.data.y[n_id[:batch_size]]) - self.cal_hsic(self.data.x[n_id[:batch_size]], out) - 
                #     self.cal_hsic(self.data.x_unlearn[n_id[:batch_size]], self.data.y_unlearn[n_id[:batch_size]]) + self.cal_hsic(self.data.x_unlearn[n_id[:batch_size]], out)
                #     )
                # loss = loss + loss1
                #out needs to be updated
                loss.backward()
                optimizer.step()
        ...
        if self.args["method"] in ["GIF", "IF", "HSIC"]:
            if self.target_model in ['GCN','SGC']:
            ...
            loss_hsic = numda * (
                    # self.cal_hsic(self.data.train_mask, self.data.y) 
                    - self.cal_hsic(self.data.train_mask, out1) 
                    + self.cal_hsic(self.data.x_unlearn[mask1], self.data.y_unlearn[mask1]) + self.cal_hsic(self.data.x_unlearn, out2) - self.cal_hsic(self.data.train_mask, out2)
                    )
            # loss += loss_hsic
            loss1 = F.nll_loss(out1[mask1], self.data.y[mask1], reduction='sum')
            loss2 = F.nll_loss(out2[mask2], self.data.y[mask2], reduction='sum')
            loss1_ = numda * (self.cal_hsic(self.data.train_mask, self.data.y) - self.cal_hsic(self.data.train_mask, out1))
            loss2_ = numda * (self.cal_hsic(self.data.x_unlearn[mask1], self.data.y_unlearn[mask1]) + self.cal_hsic(self.data.x_unlearn, out2))
            # loss1 += loss1_
            # loss2 += loss2_
...
if __name__ == '__main__':
    ...
    graphsage = HSICNodeClassifier(dataset.num_features, dataset.num_classes, args, data)
